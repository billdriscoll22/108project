...
public class Achievement {
	...
	public static void updateAchievements(User user, String type, Quiz quiz, DB db){
		if(type.equals("create")){
			...
			if(numCreated == 1){
				if(!alreadyHasAchievement(Achievement.AMATEUR_AUTHOR, user)){
				...
				if(!alreadyHasAchievement(Achievement.PROLIFIC_AUTHOR, user)){
				...
				if(!alreadyHasAchievement(Achievement.PRODIGIOUS_AUTHOR, user)){
		...
		/*Need to take into account them already getting it*/
			...
			if(results.size() == 10){
				if(!alreadyHasAchievement(Achievement.QUIZ_MACHINE, user)){
			...
			boolean hasAchievement = alreadyHasAchievement(Achievement.I_AM_THE_GREATEST, user);
			if(!hasAchievement){
	...
	private static boolean alreadyHasAchievement(String achievementName, User user){
